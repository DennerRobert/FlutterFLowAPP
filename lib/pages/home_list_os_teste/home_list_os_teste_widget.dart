...
import 'home_list_os_teste_model.dart';
export 'home_list_os_teste_model.dart';

class HomeListOsTesteWidget extends StatefulWidget {
  const HomeListOsTesteWidget({super.key});

  @override
  State<HomeListOsTesteWidget> createState() => _HomeListOsTesteWidgetState();
}

class _HomeListOsTesteWidgetState extends State<HomeListOsTesteWidget> {
  late HomeListOsTesteModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => HomeListOsTesteModel());
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      await Future.wait([
        Future(() async {
          // Chamada API
          _model.requisicaoAPI = await GetDadosUsuarioCall.call();

          if ((_model.requisicaoAPI?.succeeded ?? true)) {
            // Conectado (Online)
            _model.online = true;
            safeSetState(() {});
            _model.dudu = await GetOrdensServicoCall.call();

            if ((_model.dudu?.succeeded ?? true)) {
              FFAppState().ListaOrdemServico =
                  (_model.dudu?.jsonBody ?? '').toList().cast<dynamic>();
              safeSetState(() {});
            }
          } else {
